...
class LeRobotDatasetMetadata:
    ...
    def load_metadata(self):
        ...
        if self.ignore_features:
            logging.info(f"ignore_features: {self.ignore_features}")
            for feature in self.ignore_features:
                self.info["features"].pop(feature)
...
class LeRobotDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        self.meta = LeRobotDatasetMetadata(
            self.repo_id, self.root, self.revision, force_cache_sync=force_cache_sync, ignore_features=self.ignore_features
        )
    ...
    def _get_query_indices(
        ...
        if "subtask_end_frame_index" in self.hf_dataset.column_names:
            subtask_end_frame_index = self.hf_dataset[abs_idx]["subtask_end_frame_index"].item() + ep_start
        # query_indices = {
        ...
        for key, delta_idx in self.delta_indices.items():
            if key.startswith("observation."):
                d_list = []
                d_list.append(max(ep_start, min(ep_end - 1, abs_idx + delta_idx[0])))
                d_list.append(max(ep_start, min(ep_end - 1, subtask_end_frame_index)))
                query_indices[key] = d_list
            else:
                query_indices[key] = [max(ep_start, min(ep_end - 1, abs_idx + delta)) for delta in delta_idx]
